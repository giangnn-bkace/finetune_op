...
######### added code ##########
###############################
def classifier_block(x, weight_decay):
	'''x = conv(x, 32, 3, "cl_conv1", (weight_decay, 0))
	x = relu(x)
	x = pooling(x, 2, 2, "cl_pool1")
	x = conv(x, 32, 3, "cl_conv2", (weight_decay, 0))
	x = relu(x)
	x = pooling(x, 2, 2, "pool2")'''
...
def get_classifier_attached_vgg(weight_decay):
	...
	classifier = classifier_block(stage0_out, weight_decay)
...
def get_classifier_attached_model(weight_decay):
	...
	classifier = classifier_block(x, weight_decay)
	model = Model(inputs=[img_input], outputs=classifier)
